...
int main( int argc, char *argv[] )
{
  ...
  while ( ( in_file = readdir( FD ) ) ) 
  {
    ...
    int input_filename_length  = strlen( argv[1] ) + strlen( in_file->d_name ) ;
    int output_filename_length = strlen( argv[2] ) + strlen( in_file->d_name ) + strlen( ".enc" ) ;
    ...
    encryptFile( input_filename, output_filename );
